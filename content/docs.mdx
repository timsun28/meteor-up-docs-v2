...
**_WARNING: Windows users need to use `mup.cmd` instead in Command Prompt, as `mup` will result in unexpected behavior._**
...
module.exports = {
    ...
    app: {
        ...
        docker: {
            ...
            buildInstructions: ["RUN apt-get update && apt-get install -y imagemagick"],
...
To deploy to _different_ environments (e.g. staging, production, etc.), use separate Meteor Up configurations in separate directories, with each directory containing separate `mup.js` and `settings.json` files, and set the `app.path` field in each config to point back to your app's directory.
...
**_This Only Works if you are using the Let's Encrypt Autogenerated SSL's_**
...
_This feature is experimental._ There could be backwards breaking changes and a larger number of bugs and problems. At this time, it is not recommended to use in production.
    ...
    hooks: {
        ...
        "post.docker.setup"(api) {
            ...
            if (!username || !password) {
                throw new Error("You must provide registry login details");
            }

            return api.runSSHCommand(config.servers.one, `docker login -u ${username} -p ${password} ${registry}`);
        },
    },
};
```
